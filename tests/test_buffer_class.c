...
void test_buffer_class_constructor_string(void) {
    value_t result = test_execute_expression("Buffer(\"Hello\")");
...
void test_buffer_class_constructor_array(void) {
    value_t result = test_execute_expression("Buffer([72, 101, 108, 108, 111])");
    TEST_ASSERT_EQUAL(VAL_BUFFER, result.type);
    TEST_ASSERT_NOT_NULL(result.as.buffer);
    TEST_ASSERT_EQUAL(5, db_size(result.as.buffer));
...
void test_buffer_class_constructor_error_handling(void) {
    ...
    value_t result = test_execute_expression("Buffer([0, 255, 128])");
    TEST_ASSERT_EQUAL(VAL_BUFFER, result.type);
...
void test_buffer_class_from_hex(void) {
    value_t result = test_execute_expression("Buffer.fromHex(\"48656c6c6f\")");
    TEST_ASSERT_EQUAL(VAL_BUFFER, result.type);
    TEST_ASSERT_NOT_NULL(result.as.buffer);
    TEST_ASSERT_EQUAL(5, db_size(result.as.buffer));
...
void test_buffer_class_length_method(void) {
    value_t result = test_execute_expression("Buffer(\"Hello World\").length()");
...
void test_buffer_class_slice_method(void) {
    value_t result = test_execute_expression("Buffer(\"Hello World\").slice(6, 5).toString()");
...
void test_buffer_class_concat_method(void) {
    value_t result = test_execute_expression("Buffer(\"Hello\").concat(Buffer(\" World\")).toString()");
    TEST_ASSERT_EQUAL(VAL_STRING, result.type);
...
void test_buffer_class_to_hex_method(void) {
    value_t result = test_execute_expression("Buffer(\"Hello\").toHex()");
    TEST_ASSERT_EQUAL(VAL_STRING, result.type);
...
void test_buffer_class_to_string_method(void) {
    value_t result = test_execute_expression("Buffer(\"Hello World\").toString()");
    TEST_ASSERT_EQUAL(VAL_STRING, result.type);
    TEST_ASSERT_EQUAL_STRING("Hello World", result.as.string);
    vm_release(result);
}
...
void test_buffer_class_equals_method(void) {
    // Test equal buffers
    value_t result = test_execute_expression("Buffer(\"Hello\").equals(Buffer(\"Hello\"))");
    TEST_ASSERT_EQUAL(VAL_BOOLEAN, result.type);
    TEST_ASSERT_TRUE(result.as.boolean);
    vm_release(result);
    
    // Test different buffers
    result = test_execute_expression("Buffer(\"Hello\").equals(Buffer(\"World\"))");
    TEST_ASSERT_EQUAL(VAL_BOOLEAN, result.type);
...
void test_buffer_class_reader_method(void) {
    value_t result = test_execute_expression("Buffer(\"Hello\").reader()");
...
void test_buffer_class_method_chaining(void) {
    value_t result = test_execute_expression("Buffer(\"Hello World\").slice(0, 5).concat(Buffer(\"!\")).toString()");
    TEST_ASSERT_EQUAL(VAL_STRING, result.type);
...
void test_buffer_class_reader_integration(void) {
    ...
    value_t result = test_execute_expression("Buffer(\"H\").reader().readUint8()");
    TEST_ASSERT_EQUAL(VAL_INT32, result.type);
...
void test_buffer_class_reader_positioning(void) {
    ...
    value_t result = test_execute_expression("Buffer(\"Hello\").reader().remaining()");
    TEST_ASSERT_EQUAL(VAL_INT32, result.type);
    TEST_ASSERT_EQUAL(5, result.as.int32);
    vm_release(result);
    
    // Test position
    result = test_execute_expression("Buffer(\"Hello\").reader().position()");
    TEST_ASSERT_EQUAL(VAL_INT32, result.type);
...
void test_buffer_class_comprehensive(void) {
    ...
    value_t result = test_execute_expression("Buffer.fromHex(Buffer(\"Hello World\").slice(0, 5).toHex()).reader().readUint8()");
    TEST_ASSERT_EQUAL(VAL_INT32, result.type);
...
void test_buffer_class_vs_functional_api(void) {
    ...
    value_t string_result = test_execute_expression("Buffer(\"Hello\").toHex()");
    value_t array_result = test_execute_expression("Buffer([72, 101, 108, 108, 111]).toHex()");
...
void test_buffer_slice(void) {
    value_t result = test_execute_expression("Buffer(\"Hello\").slice(1, 3).toHex()");
    TEST_ASSERT_EQUAL(VAL_STRING, result.type);
...
void test_buffer_concat(void) {
    value_t result = test_execute_expression("Buffer(\"Hello\").concat(Buffer(\" World\")).toHex()");
    TEST_ASSERT_EQUAL(VAL_STRING, result.type);
...
void test_buffer_type_checking(void) {
    value_t result = test_execute_expression("type(Buffer(\"test\"))");
    TEST_ASSERT_EQUAL(VAL_STRING, result.type);
...
void test_buffer_reader_basic(void) {
    value_t result = test_execute_expression("BufferReader(Buffer(\"H\")).readUint8()");
    TEST_ASSERT_EQUAL(VAL_INT32, result.type);
    TEST_ASSERT_EQUAL(72, result.as.int32); // ASCII 'H'
    vm_release(result);
}
...
void test_buffer_reader_positioning(void) {
    ...
    value_t result = test_execute_expression("BufferReader(Buffer(\"Hello\")).remaining()");
    TEST_ASSERT_EQUAL(VAL_INT32, result.type);
    TEST_ASSERT_EQUAL(5, result.as.int32);
    vm_release(result);
    
    // Test position
    result = test_execute_expression("BufferReader(Buffer(\"Hello\")).position()");
    TEST_ASSERT_EQUAL(VAL_INT32, result.type);
    TEST_ASSERT_EQUAL(0, result.as.int32);
    vm_release(result);
}
