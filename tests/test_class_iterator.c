...
void test_iterator_creation_from_array(void) {
    value_t result = test_execute_expression("[1, 2, 3].iterator()");
...
void test_iterator_creation_from_range(void) {
    value_t result = test_execute_expression("(1..5).iterator()");
    TEST_ASSERT_EQUAL(VAL_ITERATOR, result.type);
    TEST_ASSERT_NOT_NULL(result.as.iterator);
    vm_release(result);
}
...
void test_iterator_type_checking(void) {
    value_t result = test_execute_expression("type([1, 2].iterator())");
...
void test_iterator_has_next_next(void) {
    ...
    value_t result = test_execute_expression("var it = [1, 2].iterator(); it.hasNext()");
    TEST_ASSERT_EQUAL(VAL_BOOLEAN, result.type);
    TEST_ASSERT_EQUAL(1, result.as.boolean); // true
    vm_release(result);
    
    result = test_execute_expression("var it = [1, 2].iterator(); it.next()");
    TEST_ASSERT_EQUAL(VAL_INT32, result.type);
    TEST_ASSERT_EQUAL(1, result.as.int32); // first element
    vm_release(result);
    
    // Test empty iterator
    result = test_execute_expression("var it = [].iterator(); it.hasNext()");
    TEST_ASSERT_EQUAL(VAL_BOOLEAN, result.type);
    TEST_ASSERT_EQUAL(0, result.as.boolean); // false
    vm_release(result);
    
    // Test range iterator
    result = test_execute_expression("var it = (1..2).iterator(); it.hasNext()");
    TEST_ASSERT_EQUAL(VAL_BOOLEAN, result.type);
    TEST_ASSERT_EQUAL(1, result.as.boolean); // true
    vm_release(result);
...
void test_iterator_is_empty(void) {
    // Test empty iterator
    value_t result = test_execute_expression("var it = [].iterator(); it.isEmpty()");
    TEST_ASSERT_EQUAL(VAL_BOOLEAN, result.type);
    TEST_ASSERT_EQUAL(1, result.as.boolean); // true - empty
    vm_release(result);
    
    // Test non-empty iterator  
    result = test_execute_expression("var it = [1, 2, 3].iterator(); it.isEmpty()");
    TEST_ASSERT_EQUAL(VAL_BOOLEAN, result.type);
    TEST_ASSERT_EQUAL(0, result.as.boolean); // false - not empty
    vm_release(result);
    
    // Test range iterator
    result = test_execute_expression("var it = (1..3).iterator(); it.isEmpty()");
    TEST_ASSERT_EQUAL(VAL_BOOLEAN, result.type);
    TEST_ASSERT_EQUAL(0, result.as.boolean); // false - not empty
    vm_release(result);
...
void test_iterator_to_array(void) {
    ...
    value_t result = test_execute_expression("var it = [1, 2, 3].iterator(); it.toArray()");
    ...
    result = test_execute_expression("var it = [].iterator(); it.toArray()");
    TEST_ASSERT_EQUAL(VAL_ARRAY, result.type);
...
void test_iterator_array_vs_range(void) {
    ...
    value_t array_result = test_execute_expression("[1, 2, 3].iterator().toArray()");
    value_t range_result = test_execute_expression("(1..3).iterator().toArray()");
    ...
    value_t array_empty = test_execute_expression("[].iterator().isEmpty()");
    value_t range_empty = test_execute_expression("(5..<5).iterator().isEmpty()");
...
void test_iterator_state_progression(void) {
    ...
    value_t result = test_execute_expression("var it = [42, 99].iterator(); var first = it.next(); var has_next = it.hasNext(); var second = it.next(); var done = it.hasNext(); first");
    TEST_ASSERT_EQUAL(VAL_INT32, result.type);
...
void test_iterator_edge_cases(void) {
    ...
    value_t result = test_execute_expression("var it = [42].iterator(); it.toArray()");
    TEST_ASSERT_EQUAL(VAL_ARRAY, result.type);
    ...
    result = test_execute_expression("var it = (1..<2).iterator(); it.toArray()");
    TEST_ASSERT_EQUAL(VAL_ARRAY, result.type);
...
void test_iterator_method_chaining(void) {
    ...
    value_t result = test_execute_expression("var arr = [1, 2, 3].iterator().toArray(); arr");
    TEST_ASSERT_EQUAL(VAL_ARRAY, result.type);
    TEST_ASSERT_EQUAL(3, da_length(result.as.array));
    vm_release(result);
}
