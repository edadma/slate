...
// Helper function to run while loop code and return result
...
void test_while_loops_with_do_multiline(void) {
    ...
    result = run_while_test("var sum = 0\n"
                           ...
                           "sum");
    TEST_ASSERT_EQUAL_INT(VAL_NUMBER, result.type);
    TEST_ASSERT_EQUAL_DOUBLE(15.0, result.as.number); // 1+2+3+4+5 = 15
    vm_release(result);
    ...
    result = run_while_test("var product = 1\n"
                           ...
                           "product");
    TEST_ASSERT_EQUAL_INT(VAL_NUMBER, result.type);
    TEST_ASSERT_EQUAL_DOUBLE(24.0, result.as.number); // 1*2*3*4 = 24
    vm_release(result);
...
void test_while_loop_edge_cases(void) {
    ...
    result = run_while_test("var x = 1\n"
                           ...
                           "x + y");
    TEST_ASSERT_EQUAL_INT(VAL_NUMBER, result.type);
    TEST_ASSERT_EQUAL_DOUBLE(11.0, result.as.number); // x=4, y=7 -> 4+7=11
    vm_release(result);
    ...
    result = run_while_test("var total = 0\n"
                           ...
                           "total");
    TEST_ASSERT_EQUAL_INT(VAL_NUMBER, result.type);
    TEST_ASSERT_EQUAL_DOUBLE(6.0, result.as.number); // 3 * 2 = 6 iterations
    vm_release(result);
}
