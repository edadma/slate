 ...
 */
static inline db_internal* db_meta(db_buffer buf) { return (db_internal*)((char*)(buf) - sizeof(db_internal)); }
...
db_buffer db_new(size_t capacity) { return db_alloc(capacity); }
...
db_buffer db_new_from_owned_data(void* data, size_t size, size_t capacity) {
    DB_ASSERT((data || (size == 0 && capacity == 0)) &&
...
db_buffer db_read_file(const char* filename) {
    if (!filename)
        return NULL; // Allow NULL filename as runtime error
...
db_buffer db_from_hex(const char* hex_string, size_t length) {
    if (!hex_string || length % 2 != 0)
        return NULL; // Runtime validation
...
int db_builder_append_uint32_be(db_builder builder, uint32_t value) {
    ...
    uint8_t bytes[4] = {(uint8_t)((value >> 24) & 0xFF), (uint8_t)((value >> 16) & 0xFF),
...
int db_builder_append_uint64_be(db_builder builder, uint64_t value) {
    ...
    uint8_t bytes[8] = {(uint8_t)((value >> 56) & 0xFF), (uint8_t)((value >> 48) & 0xFF),
...
uint32_t db_read_uint32_le(db_reader reader) {
    ...
    const uint8_t* ptr = (const uint8_t*)(reader->buf + reader->position);
    uint32_t value = (uint32_t)ptr[0] | ((uint32_t)ptr[1] << 8) | ((uint32_t)ptr[2] << 16) | ((uint32_t)ptr[3] << 24);
    reader->position += 4;
...
uint64_t db_read_uint64_le(db_reader reader) {
    ...
    const uint8_t* ptr = (const uint8_t*)(reader->buf + reader->position);
    uint64_t value = (uint64_t)ptr[0] | ((uint64_t)ptr[1] << 8) | ((uint64_t)ptr[2] << 16) | ((uint64_t)ptr[3] << 24) |
...
uint64_t db_read_uint64_be(db_reader reader) {
    ...
    const uint8_t* ptr = (const uint8_t*)(reader->buf + reader->position);
    uint64_t value = ((uint64_t)ptr[0] << 56) | ((uint64_t)ptr[1] << 48) | ((uint64_t)ptr[2] << 40) |
        ((uint64_t)ptr[3] << 32) | ((uint64_t)ptr[4] << 24) | ((uint64_t)ptr[5] << 16) | ((uint64_t)ptr[6] << 8) |
                    (uint64_t)ptr[7];
    reader->position += 8;
